...
<div class="contentpanel">
    ...
    <div class="panel panel-default">
                    ...
                    <div class="panel-heading">
                        ...
                        <div class="stat">
                            <div class="row">
                                ...
                                <div class="col-xs-8">
                                    <small class="stat-label">物流出量</small>
                                    <h1><?php echo ($buy_out); ?></h1>
                                </div>
                            </div><!-- row -->
                        </div><!-- stat -->
                    </div><!-- panel-heading -->
                    ...
                    <div class="panel-heading">
                        ...
                        <div class="stat">
                            <div class="row">
                                ...
                                <div class="col-xs-8">
                                    <small class="stat-label">自提数量</small>
                                    <h1><?php echo ($self_out); ?></h1>
                                </div>
                            </div><!-- row -->
                        </div><!-- stat -->
                    </div><!-- panel-heading -->
        ...
        <div class="panel-body">
            ...
            <form action="<?php echo U('Goods/order_sr','p='.$page.'&dotype='.$dotype);?>" class="form-inline mb15" role="form">
                ...
                <div class="form-group">
                    <input type="text" name='name' id="username" class="form-control input-sm" value="<?php echo ($name); ?>" placeholder="请输入收货用户"/>
                </div>
                <div class="form-group">
                ...
                <div class="btn btn-default btn-sm" id="getTransn" style="padding-left: 20px;background-color: #2f27b2;color: white;">导出未发货已获取单号数据</div>
            </form>
                ...
                <table class="table table-bordered" id="datatables">
                    <thead>
                        <tr>
                            ...
                            <th width="4%">收货人</th>
                            <th width="16%">地址</th>
                            <!-- <th width="6%">同周边订单号</th> -->
                            <!-- <th width="6%">同168订单号</th> -->
                            <th width="5%">数量</th>
                            <th width="6%">物流公司</th>
                            <th width="8%">物流单号</th>
                            <th width="6%">申请时间</th>
                    ...
                    <?php if(is_array($list["list"])): $i = 0; $__LIST__ = $list["list"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
                            ...
                            <!-- <td><?php echo ($v["same"]); ?></td> -->
                            <!-- <td><?php echo ($v["same_pro"]); ?></td> -->
                            ...
                            <td>
								...
								<?php echo ($v['status']==4?'自提 ':''); echo ($v['sure']==1?'-已确认 ':''); ?>
								<?php echo ($v['status']==5?'已取消 ':''); ?>
                                <?php echo ($v['status']==6?'已确认 ':''); ?>
                            </td>
                            <td><?php echo ($v["trans_pay"]); ?></td>
                            <!--<td><?php echo (getAddrInfo($v['addr'],idcard,$v["addr"])); ?></td>-->
                            <td><?php echo ($v["remakes"]); ?></td>
                            <td>
                                <?php if($v["have_pay"] == 0): ?><span style="color: #bf5124;"> 未支付</span>
                                    <?php elseif($v["have_pay"] == 2): ?>
                                    <span style="color: #83241a;">已取消</span>
                                    <?php elseif($v["have_pay"] == 3): ?>
                                    <?php if(($v["sure"]) == "0"): ?><a class='btn btn-xs btn-success' href="javascript:sure(<?php echo ($v["id"]); ?>,1);">确认出货</a>
                                       <a class='btn btn-xs btn-success' href="javascript:sure(<?php echo ($v["id"]); ?>,3);">取消提货</a><?php endif; ?>
                                    <?php else: ?>
                                    ...
                                    <?php if(($v["status"]) == "1"): if(($v["sure"]) == "0"): ?><a class='btn btn-xs btn-info' href="javascript:doTrans(<?php echo ($v["id"]); ?>);">发货</a>
                                        <!--<a class='btn btn-xs btn-info' href="javascript:doOption(<?php echo ($v["id"]); ?>,3);">发货</a>-->
                                        <a class='btn btn-xs btn-success' href="javascript:doOption(<?php echo ($v["id"]); ?>,4);">自提</a>
                                        
                                        <a class='btn btn-xs btn-danger' href="javascript:desetord(<?php echo ($v["id"]); ?>,5);">取消发货</a><?php endif; ?>
                                      <!--  <?php if(($v["sure"]) == "0"): ?><a class='btn btn-xs btn-info' href="javascript:getTrsn(<?php echo ($v["id"]); ?>);">获取订单号</a><?php endif; ?>--><?php endif; endif; ?>
...
<script>
    ...
    function desetord(id, status) {
        var configURL = "<?php echo U('Goods/calloffOrder');?>";
        var data = {id: id, status: status};
        layer.open({
            type : 0,
            title : '提示',
    ...
    function sure(id, sure) {
        var configURL = "<?php echo U('Goods/suresend');?>";
        var data = {id: id, sure: sure};
        ajaxRt(configURL, data);
    }
   ...
   /* function getTrsn(id) {
        var configURL = "<?php echo U('zop/getinfo');?>";
        var data = {id: id};
        layer.alert(11);
        ajaxRt(configURL, data);
    }*/
    function getTrsn(id) {
        ...
        layer.open({
            ...
            shade: 0.6,
            area: ['380px', '90%'],
            content: "<?php echo U('zop/getinfo');?>" //iframe的url
        });
    }
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        if (res.status !== 1) {
                            ...
                            window.location.href="<?php echo U('Backet/Docsv/export_ordersr');?>?name="+name+"&phone="+phone+"&start="+start+"&ends="+ends+"&status="+status;
                        }
                    }
                });
                layer.close(index);
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        if (res.status !== 1) {
                            ...
                            window.location.href="<?php echo U('Backet/Doexc/dotranZop');?>";
                        }
                    }
                });
                layer.close(index);
