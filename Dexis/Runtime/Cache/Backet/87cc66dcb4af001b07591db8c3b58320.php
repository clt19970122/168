    ...
    <div class="panel panel-default">
        <div class="panel-heading">
            <h3 class="panel-title">
                商品列表
                <!--a href="<?php echo U('Goods/orderexport','model=goods&type='.$type);?>" class="btn btn-primary btn-xs pull-right" target="_blank">导出</a-->
            </h3>
            <p>
                <a href="<?php echo U('Goods/indexadd');?>" class="btn btn-xs btn-info">添加商品</a>
            </p>
        </div>
        <div class="panel-body">

            <form action="<?php echo U('Goods/index','p='.$page);?>" class="form-inline mb15" role="form">
                <div class="form-group">
                    <input type="text" name='name' class="form-control input-sm" value="<?php echo ($get["name"]); ?>" placeholder="请输入商品名称"/>
                </div>
                <div class="form-group">
                ...
                <div class="form-group">
                    <input type="text" name='start' class="form-control input-sm" id="single_start" value='<?php echo ($get["start"]); ?>' placeholder="请选择上架开始时间"/>
                </div>
                <div class="form-group">
                    <input type="text" name='ends' class="form-control input-sm" id="single_end" value='<?php echo ($get["ends"]); ?>' placeholder="请选择上架结束时间"/>
                </div>
            ...
            <div class="table-responsive">
                <table class="table table-bordered" id="datatables">
                    ...
                    <?php if(is_array($list["list"])): $i = 0; $__LIST__ = $list["list"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
                            ...
                            <td><?php echo (getProductName($v['catid'],$v["catid"])); ?></td>
                            <!--<td><?php echo (getQusName($v['type'],$name)); ?></td>-->
                            <td><?php echo ($v["price"]); ?></td>
                            <td><?php echo ($v['ind']>0?'是':'否'); ?></td>
                            <td><?php echo ($v['zplan']>0?'是':'否'); ?></td>
                            <td><?php echo ($v['status']>0?'销售中':'已下架'); ?></td>
                            <td><?php echo (date('Y/m/d H:i:s',$v["times"])); ?></td>
                            <td>
                                <a href="<?php echo U('Goods/indexedit','id='.$v['id']);?>" class='btn btn-xs btn-info'>修改</a>
                                <?php if(($v["status"]) == "1"): ?><a href='javascript:change(<?php echo ($v["id"]); ?>,0);' class='btn btn-xs btn-danger'>下架</a><?php endif; ?>
                                <?php if(($v["status"]) == "0"): ?><a href='javascript:change(<?php echo ($v["id"]); ?>,1);' class='btn btn-xs btn-success'>上架</a>
                                <a href='javascript:dels(<?php echo ($v["id"]); ?>);' class='btn btn-xs btn-danger'>删除</a><?php endif; ?>
                            </td>
...
<script>
    ...
    function change(id, type) {
        var configURL = "<?php echo U('Core/edits', 'model=goods');?>";
        var data = {ids: id, status: type};
    ...
    function dels(id) {
        ...
        var configURL = "<?php echo U('Core/dels', 'model=goods');?>";
        var data = {id: id};
        ajaxRt(configURL, data);
    }
