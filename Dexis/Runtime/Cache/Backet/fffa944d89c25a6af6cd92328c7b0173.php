...
<div class="contentpanel">
    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            ...
            <form action="<?php echo U('Goods/order','type='.$type.'&p='.$page);?>" class="form-inline mb15" role="form">
                ...
                <div class="form-group">
                    <select name="gid" id="gid" class="form-control input-sm">
                        ...
                        <?php if(is_array($goods)): $i = 0; $__LIST__ = $goods;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>" <?php if($vo['id'] == $get['gid']): ?>selected="selected"<?php endif; ?>><?php echo ($vo["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                    </select>
                </div>
                <div class="form-group">
                    <select name="paytype" id="payway" class="form-control input-sm">
                        <option value="">全部支付方式</option>
                        <option value="1" <?php if(1 == $get['paytype']): ?>selected="selected"<?php endif; ?>>微信支付</option>
                        <option value="4"<?php if(4 == $get['paytype']): ?>selected="selected"<?php endif; ?>>余额支付</option>
                    </select>
                </div>
                <div class="form-group">
                    <select name="buy_level" id="level" class="form-control input-sm">
                        <option value="">全部等级</option>
                        <?php if(is_array($level)): $i = 0; $__LIST__ = $level;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><option value="<?php echo ($v["id"]); ?>" <?php if($v['id'] == $get['buy_level']): ?>selected="selected"<?php endif; ?>><?php echo ($v["name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; ?>
                    </select>
                </div>
                <div class="form-group">
            ...
            <div class="table-responsive">
                <table class="table table-bordered" id="datatables">
                    ...
                    <?php if(is_array($list["list"])): $i = 0; $__LIST__ = $list["list"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
                            ...
                            <td><?php echo ($v["sn"]); ?></td>
                            <td onclick="checkPassword(this,<?php echo ($v['uid']); ?>)"><?php echo (yc_phone(getUserInf($v['uid'],'phone',$p))); ?></td>
                            ...
                            <td>
                                ...
                                <?php echo ($v['status']==6?'退单处理':''); ?>
                            </td>
                            <td><?php echo (getpaytype($v["paytype"])); ?></td>
                            <td><?php echo ($v["remakes"]); ?></td>
                            <td><?php echo (getUserLevel($v["buy_level"])); ?></td>
                            <!--td>
                            ...
                            <td>
                                ...
                                <?php if($v["status"] == 0): ?><a href="javascript:dels(<?php echo ($v["id"]); ?>);" class='btn btn-xs btn-danger'>删除</a><?php endif; ?>
                                <?php if($v["status"] == 1): ?><a href="javascript:chargeback(<?php echo ($v["id"]); ?>);" class='btn btn-xs btn-success'>退单</a><?php endif; ?>
                            </td>
...
<script>
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        if (res.status !== 1) {
                            ...
                            window.location.href="<?php echo U('Backet/Doexc/doExcel_Order');?>?sn="+sn+"&username="+username+"&phone="+phone+"&tjid="+tjid+"&start="+start+"&ends="+ends+"&status="+status+"&type="+type+'&paytype='+paytype+'&gid='+gid+'&buy_level='+buy_level;
                        }
                    }
                });
                layer.close(index);
    ...
    function chargeback(obj){
        console.log(obj);
            $.ajax({
                url: "<?php echo U('Goods/DoReturnByOrder');?>", type: 'post',
                data: {sn:obj}, dataType: "json",
                success: function (res) {
                    if (res.status !== 1) {
                        return layer.msg(res.msg);
                        location.reload();
