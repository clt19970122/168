...
<div class="contentpanel">
    ...
    <div class="panel panel-default">
        ...
        <div class="panel-body">
            <form action="<?php echo U('Fina/index');?>" class="form-inline mb15" role="form">
                <div class="form-group">
                    <label class="sr-only">Email address</label>
                    <input type="text" name='sn' class="form-control input-sm" id ='sn' value="<?php echo ($get["sn"]); ?>" placeholder="请输入订单编号"/>
                </div>
                <div class="form-group">
                    <label class="sr-only">Email address</label>
                    <input type="text" name='realname' class="form-control input-sm" id="realname" value="<?php echo ($get["realname"]); ?>" placeholder="请输入系统名称"/>
                </div>
                <div class="form-group">
                    <input type="text" name="name" class="form-control input-sm" id="name" value="<?php echo ($get["name"]); ?>" placeholder="请输入持卡人名"/>
                </div>
                <div class="form-group">
                    <input type="text" name='start' class="form-control input-sm"  value="<?php echo ($get["start"]); ?>" id="single_start" placeholder="请选择开始时间"/>
                </div>
                <div class="form-group">
                    <input type="text" name='ends' class="form-control input-sm" value="<?php echo ($get["ends"]); ?>" id="single_end" placeholder="请选择结束时间"/>
                </div>
                <div class="form-group">
                ...
                <div class="btn btn-default btn-sm" id="doExcel1" style="padding-left: 20px;background-color: #2f27b2;color: white;">导出可打款数据</div>
                <div class="btn btn-default btn-sm" id="doExcel2" style="padding-left: 20px;background-color: #2f27b2;color: white;">导出现金可打款数据</div>
            </form>
            ...
            <div class="table-responsive">
                <table class="table table-bordered" id="datatables">
                    ...
                    <?php if(is_array($list["list"])): $i = 0; $__LIST__ = $list["list"];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$v): $mod = ($i % 2 );++$i;?><tr>
                            <td><?php echo ($v["id"]); ?></td>
                            <td><?php echo ($v["sn"]); ?></td>
                            <td><?php echo (getUserInf($v['usid'],'nickname',$v["usid"])); ?></td>
                            <td><?php echo (getUserLevel(getUserInf($v['usid'],'level',$v["usid"]))); ?></td>
                            <td><?php echo ($v["money"]); ?></td>
                            <td><?php echo ($v["name"]); ?></td>
                            <td><?php echo (getBankName($v['bank'],'name',$v["bank"])); ?></td>
                            <td><?php echo ($v["bankcode"]); ?></td>
                            <td><?php echo (date('Y/m/d H:i:s',$v["times"])); ?></td>
                            <td><?php echo (payRemake($v["sn"])); ?></td>
                            <td>
                                <?php echo ($v['status']==0?'待支付':''); ?>
                                <?php echo ($v['status']==1?'已支付':''); ?>
                                <?php echo ($v['status']==2?'已退回':''); ?>
                            </td>
                            <td>
                                <?php if(($v["status"]) == "0"): ?><a href="javascript:setRolls(<?php echo ($v["id"]); ?>);" class='btn btn-xs btn-success'>退回</a>
                                <a href="javascript:void(0);" class='btn btn-xs btn-danger' onclick="doMakeMoney(<?php echo ($v["id"]); ?>)">打款</a>
                                <a href="javascript:void(0);" class='btn btn-xs btn-warning' onclick="dobycard(<?php echo ($v["id"]); ?>)">银行卡支付</a><?php endif; ?>
                            </td>
    ...
    <input type="hidden" id="ssssssssss"  value="<?php echo ($type); ?>">
</div>
            ...
            <div class="panel-body">
                <form role="form" name="trans">
                    ...
                    <div class="form-group">
                        ...
                        <!--<input type="text" class="form-control" id="remake" name="remake"/>-->
                    </div>
...
<script>
    ...
    function setRolls(id) {
        var urls = "<?php echo U('Fina/doRolls');?>";
        var data = {ids: id};
        ajaxRt(urls, data);
    }

    function setOrder(id) {
        var urls = "<?php echo U('Core/edits', 'model=money_draw');?>";
        var data = {ids: id, status: 1};
        ajaxRt(urls, data);
    }

    // 打款方法弹出框
    function doMakeMoney(id) {
        $("#makeMoney").modal('toggle');
        $.ajax({
            type : "POST", //提交方式
            url : "<?php echo U('fina/getOneData');?>",//路径
            data : {
                "id":id
            },//数据，这里使用的是Json格式进行传输
            success : function(result) {
                $('#uinfo').text(result.data.name);
                $('#pmoney').html(result.data.money);
                $('#ids').val(result.data.id);
            }
        });
    }
    ...
    function doPayss() {
        ...
        var configURL = "<?php echo U('pay/doPayLogic');?>";

     /*   var data=[];
        data.type =type;
        data.pwd=pwd;
        data.id =id;*/
        var data = {id: id,pwd: pwd, type: type, remake: remake};
        console.log(data);
    ...
    function dobycard(id) {
        var data = {id: id};
        var configURL = "<?php echo U('fina/doByCard');?>";
        ajaxRt(configURL,data)
    }
...
<script>
    $("#doExcel").on('click',function(){
        ...
        var status =$('#ssssssssss').val();
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        if (res.status !== 1) {
                            ...
                            window.location.href="<?php echo U('Backet/Docsv/export_fina');?>?sn="+sn+"&name="+name+"&phone="+phone+"&start="+start+"&ends="+ends+"&type="+status;
                        }
                    }
                });
                layer.close(index);
         ...
         // window.location.href="<?php echo U('Backet/Doexc/doExcel_fina');?>?sn="+sn+"&name="+name+"&phone="+phone+"&start="+start+"&ends="+ends+"&type="+status;
       //  window.location.href="<?php echo U('Backet/Docsv/export_fina');?>?sn="+sn+"&name="+name+"&phone="+phone+"&start="+start+"&ends="+ends+"&type="+status;
    });
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        if (res.status !== 1) {
                            ...
                            window.location.href="<?php echo U('Backet/Doexc/doexcel_by_plan_can_do');?>?sn="+sn+"&name="+name+"&phone="+phone+"&start="+start+"&ends="+ends+"&type="+status;
                        }
                    }
                });
                layer.close(index);
                ...
                $.ajax({
                    ...
                    success: function (res) {
                        if (res.status !== 1) {
                            ...
                            window.location.href="<?php echo U('Backet/Doexc/doexcel_cash_pay');?>?sn="+sn+"&name="+name+"&phone="+phone+"&start="+start+"&ends="+ends+"&type="+status;
                        }
                    }
                });
                layer.close(index);
