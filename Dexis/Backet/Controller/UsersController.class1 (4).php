...
//                    $res_data[$k]['change_nums'] =$v['models']=='ORDER'?'<span style="color: #3ab22c;font-size: 18px">+' .$nums_data['nums'].'</span>': '<span style="color: #b2282c;font-size: 18px">-' .$nums_data['nums'].'</span>';
        ...
        $nums_data =$acc_nums->where($nums_where)->page($p2,20)->order('id desc')->select();
...
//                $nums_res[$k]['do'] =$v['type']==21?'转货':($v['type']==23?'提货':'扣除');
                $nums_res[$k]['do'] =$v['type']==21?'转货':($v['type']==23?'提货':'后台转');//21 23 24
               /* if($v['type']==21){
                    if($v['uid'] == $id){
...
//        $pick_num =M('order_drs')->where($pick_where)->sum('nums');
            ...
            //计算金额
            $this_year = $acc_nums->where(['type' => array('lt', 20), 'time' => array('gt', strtotime('2019-1-1')), 'aboutid' => 0, 'status' => 1])->field('sn,nums')->select();
...
//            $where_nums2['time'] = array('gt', $attime);
    ...
    public function userLlCgList(){
        ...
        //获取用户列表数据
        $res_data =$acc_record->where($where)->limit($Page->firstRow.','.$Page->listRows)->order('id desc')->select();
...
//        $update["totalpoints"]= array("exp", "totalpoints+" . $get["stock"]);
   ...
   public function addUser_contect($uid=1, $now, $level=0){
        ...
        if($now==1){
            $now =file_get_contents('contectuid.txt');
        }
...
//        var_dump($this->countLine('15b10af74c4c44','a',array()));
    ...
    public function add_contect($uid=1, $now, $level=0){
        $account =M('account');
        $acc_contect =M('acc_contect');
        if($now==1){
            $now =file_get_contents('contectuid.txt');
        }
        $last_id =$account->field('id')->order('id desc')->find();
        //最后一条数据 <现在数据
        if($last_id['id']<$now){
            $this->ajaxReturn(['status'=>0,'msg'=>'更新完成']);
        }
        //当前用户
        $user_data =$account->where(['id'=>$uid])->field('id,recid,sysid,level')->find();
        if($user_data && $user_data['level']>=0){
            $acc_contect->where(['rold_id'=>$now])->find();
            if(!$acc_contect){
                //上级代理信息
                $ancestor = $account->where(['sysid' => $user_data['recid']])->field('id,level')->find();
                //存在上级
