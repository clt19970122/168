...
class UsersController extends CommController {
    ...
    public function index() {
        ...
        $get["phone"] != null ? $where["phone"] = array("like", "%" . trim($get["phone"]) . "%") : null;
        $get["nickname"] != null ? $where["nickname"] = array("like", "%" . trim($get["nickname"]) . "%") : null;
        $get["name"] != null ? $where["name"] = array("like", "%" . trim($get["name"]) . "%") : null;
        ...
        foreach ($list['list'] as $k=>$v){
            ...
            if($v['recid']!=null||$v['recid']!=0){
                ...
                $list['list'][$k]['frozenMoney'] =$money=accountFrozen($v['id']);
            }
        }
...
//            $user_info[$k]['innums']=$insum1;
        ...
        $this->assign('get',$get);
        $this->assign("level", $acc_level->select());
        $this->display('Users/getuserinandout');
    }
 ...
 */
    public function frozen(){
        ...
        $account = M("account");
        $get = I("get.");
        $name=getUserInf($get['ssid'],'name');
        $data =$account->where(['id'=>$get['ssid']])->find();
        $money=accountFrozen($get['ssid']);
        $where['uid']=$get['ssid'];
        $count = poPage($frozen, $where);
    ...
    public function frozenMoney(){
        ...
        $data =$account->where(['id'=>$get['uid']])->find();
        $num = $get['frozen_money'];
        $rule = '/^(([1-9][0-9]*)|(([0]\.\d{1,2}|[1-9][0-9]*\.\d{1,2})))$/';
        ...
        if($get['type']==1){
            if($data['money']<$get['frozen_money']){
                $status=2;
                $msg ='冻结失败，用户余额不足';
                $this->ajaxReturn(['status'=>$status,'msg'=>$msg]);
            }
            $money=bcsub($data['money'],$get['frozen_money'],2);
            $moneys['money']=$money;
            $udi=$account->where(['id'=>$get['uid']])->save($moneys);
        }else{
            $money=accountFrozen($get['uid']);
            if($money<$get['frozen_money']){
                $status=4;
                $msg ='解冻失败，冻结金额不足';
                $this->ajaxReturn(['status'=>$status,'msg'=>$msg]);
            }
            $money=bcadd($data['money'],$get['frozen_money'],2);
            $moneys['money']=$money;
            $udi=$account->where(['id'=>$get['uid']])->save($moneys);
        }
        $add_data = [
            'sn' =>date('YmdHis', time()) . rand(10, 100),
            'frozen_money' => $get['frozen_money'],
            'money' => $get['money'],
            'd_money' => $money,
            'remark' => $get['content'],
            'type' => $get['type'],
     ...
     *展示优惠
     */
    public function show_stage($uid){
        $stages=M('acc_stages');
        $discount =M('acc_st_discount');
        $info_data =$stages->alias('s')
            ->where(['s.uid'=>$uid])
            ->select();
        foreach($info_data as $k=>$v){
            $dis_arr =$discount->where(['stages_id'=>$v['id']])->select();
            $info_data[$k]['dis_arr']=$dis_arr;
        }
        return $info_data;
    }
     ...
     *添加分期记录
     */
    public function add_stages_list(){
        $account =M('account');
        $acc_stages=M('acc_stages');//分期表
        $acc_discount=M('acc_st_discount');//优惠活动表
        //获取数据
        $data =I('get.');
        $user_data =$account->where(['id'=>$data['uid']])->find();
        $add['uid'] =$data['uid'];
        $add['level'] =$user_data['level'];
        $add['payway'] =$data['payway'];
        $add['every_pay'] =$data['money']/$data['payway'];
        $add['pay_time'] =date('Y-m-0 0:0:0',strtotime( '+'.$data['payway'].'months'));
        $add['add_time'] =time();
        //添加
        $stage_id= $acc_stages->add($add);
        //活动表
        $add_data =array();
        if($data['dis_name'] != null) {
            $nums = count($data['dis_name']);
            for ($i = 0; $i < $nums; $i++) {
                //添加优惠活动
                if ($data['dis_name'][$i] != null) {
                    $add_data[$i]['discount_name'] = $data['dis_name'][$i];
                    $add_data[$i]['discount_num'] = $data['dis_val'][$i];
                    $add_data[$i]['change_level'] = $data['dis_level'][$i];
//                   $add_data[$i]['discount_var'] = $data['dis_pri'][$i];//购买优惠的价
                    $add_data[$i]['discount_buy_num'] = $data['dis_pri'][$i];//购买优惠的盒
                    $add_data[$i]['start_time'] = $data['start'][$i];
                    $add_data[$i]['end_time'] = $data['end'][$i];
                    $add_data[$i]['stages_id'] = $stage_id;
                    $add_data[$i]['uid'] = $data['uid'];
                    $add_data[$i]['time'] = time();
                }
            }
        }
        if($add_data !=null){
            $acc_discount->addAll($add_data);
        }
        $this->redirect('users/indexview',array('id'=>$data['uid']));
    }
    /**
     * 一键冻结操作
     */
    public function frozenMoneyall(){
        $frozen = M("frozen");
        $account = M("account");
        $get = I("get.");
        if($get['pwd']!='168dj'){
        ...
        if($get['type']==1){
            ...
            $money=bcsub($data['money'],$data['money'],2);
            $moneys['money']=$money;
            $udi=$account->where(['id'=>$get['uid']])->save($moneys);
            $add_data = [
                'sn' =>date('YmdHis', time()) . rand(10, 100),
                'frozen_money' => $data['money'],//冻结或者解冻金额
                'money' => $get['money'],//冻结前金额
                'd_money' => $money,//冻结或者解冻后金额
                'remark' => $get['content'],
                'type' => $get['type'],
                'uid' => $get['uid'],
                'time' => time(),
            ...
            $money=accountFrozen($get['uid']);//用户冻结金额
            if($money==0){
                $status=4;
                $msg ='用户没有冻结金额';
                $this->ajaxReturn(['status'=>$status,'msg'=>$msg]);
            }
            $addmoney=bcadd($data['money'],$money,2);//加上之后的金额
            //  var_dump($addmoney);exit;
            $moneys['money']=$addmoney;
            $udi=$account->where(['id'=>$get['uid']])->save($moneys);
            $add_data = [
                'sn' =>date('YmdHis', time()) . rand(10, 100),
                'frozen_money' => $money,//冻结或者解冻金额
                'money' => $get['money'],//冻结前金额
                'd_money' => $addmoney,//冻结或者解冻后金额
                'remark' => $get['content'],
                'type' => $get['type'],
                'uid' => $get['uid'],
                'time' => time(),
   ...
   /**
    ...
    public function examine(){
        ...
        $udi=$account->field('phone')->where(['id'=>$uid])->find();
        if($udi){
            $status=1;
            $this->ajaxReturn(['status'=>$status,'msg'=>$udi['phone'],'id'=>$uid]);
        }
    }
    ...
    public function doExcel(){
        ...
        $pwd=md5($pwd);
        if($pwd!='8b5ec81d55775af34b980a3a9b8b1260'){
            // if($pwd!='88345f319f40f3381151c9ec83058901﻿'){
            $status=2;
            $msg ='密码错误';
            $this->ajaxReturn(['status'=>$status,'msg'=>$msg]);
        }else{
...
//        var_dump($list);
    ...
    public function doturn(){
        ...
        $acc_nums =M('acc_nums');
        if($type==1){
            $data=$acc_nums->where(['id'=>$id])->find();
            //被转用户的信息
            $resinfo =$account->where(['id'=>$data['aboutid']])->find();
            $saves['stock']=$resinfo['stock']+$data['nums'];
            $saves['points']=$resinfo['points']+$data['nums'];
            $saves['totalpoints']=$resinfo['totalpoints']+$data['nums'];
            $rs=$account->where(['id'=>$data['aboutid']])->save($saves);
            //检查积分是否足够升级
            $code_data=$account->where(['id'=>$data['aboutid']])->find();
            //根据积分来修改等级
            ...
            $datas=$acc_nums->where(['id'=>$id])->save($add_nums);
            if($datas) {
                $spread_price =getTheOryMoney($data['uid'],$resinfo['id']);
                //单盒*数量
                $getpro=$spread_price *$data['nums'];
                addProfitList($getpro,$data['uid'],$data['sn'],3);
                return get_op_put(1, "转入完成", U('users/doturn_list'));
            }
            else {
            ...
            $data=$acc_nums->where(['id'=>$id])->find();
            $resinfo =$account->where(['id'=>$data['uid']])->find();
            $saves['stock']=$data['nums']+$resinfo['stock'];
            $resinfo =$account->where(['id'=>$data['uid']])->save($saves);
            $add_nums=[
                'status'=>3,
            ];
            $data=$acc_nums->where(['id'=>$id])->save($add_nums);
            if($data) {
                return get_op_put(1, "驳回成功", U('users/doturn_list'));
            }
            else {
                return get_op_put(0, "转入失败");
            }
    ...
    public function Buysend(){
        ...
        $account =M('account');
        $where['id'] =$id;
        $user_info =$account->where($where)->find();
        if($user_info) {


            if ($user_info['buy_send'] == 0) {
                $save['buy_send'] = 1;
            } else {
                $save['buy_send'] = 0;
            }
            $res = M('account')->where(['id' => $id])->save($save);
            if ($res) {
                return get_op_put(1, "设置成功", '', 1);
            } else {
                return get_op_put(0, "设置失败");
            }
        }else{
            return get_op_put(0, "输入信息错误");
        }
    }
    /**
     * 减肥营调查列表
    ...
    public function weight() {
        $weight = M("weight");
        $where=[];
        $list = poPage($weight, $where);
        foreach ($list['list'] as $k=>$v){
            $list['list'][$k]['count']=json_decode($v['count']);
        }
        $this->assign("list", $list);
        $this->display();
    }
    ...
    public function freezeAccount(){
        ...
        $order_sr = M("order_sr");
        $account = M("account");
        $where['status'] =array('in','7');//已逾期的

        $res = $order_sr->where($where)->select();

        foreach($res as $key =>$value){
            $user_info[$key] =$account->where(['id'=>$value['uid']])->save(['status'=>0]);
        }
        return get_op_put(1, "success",'设置成功',U('users/index'));
    }
}
