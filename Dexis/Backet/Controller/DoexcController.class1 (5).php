...
//        $this->exportExcel($datas,date(Y-m-d).'导出数据',$cellName,'sheetname');
    ...
    public function doExcel_plans(){
        ...
        $title=array('序号','联创','出货人','推荐用户','下单用户','订单编号','姓名','身份证','手机','金额','打款状态','申请时间','库存','已提现金额','账户余额');
        $cellName=array('no','topuser','outname','low_user','nickname','sn','name','idcard','phone','money','status','time','stock','draw','resmoney');
        ...
        foreach ($data as $k=>$v){
            ...
            $datas[$k]['resmoney'] =getUserInf($v['uid'],'money');
...
//添加记录
        ...
        $msg='';
        if($get["level"]){
            $msg .=$user_info[0]['level'];
        }
        addExcel_list($user_info['ch_name'],$msg.'用户进货量数据导出');
...
//                echo 11;die;
        ...
        $title=array('序号','申请用户','订单号','金额','状态','时间','等级');
        $cellName=array('id','name','sn','money','status','times','level');
