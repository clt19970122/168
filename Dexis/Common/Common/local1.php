 ...
 * @param type $res
 */
function cgUserMoney($id, $money, $type, $res, $sn = 0,$iswx=true) {
    $account = M("account");
 ...
 * @param $nums
 */
function changeUserLevel($id, $nums){
    $account = M("account");
    $acc_level =M('acc_level');
    $where['months'] =array('elt',$nums);
...
//    $user_info =$account->where(['id'=>$uid])->find();
    #用户成本价
//    $good_level = M('goods_level');
//    $level_money =$good_level->where(['lid'=>$user_info['level'],'gsn'=>'415244730815416283'])->find();
//    $cost =$level_money['price'];
//    var_dump($cost);
    #
    #统计出货量
    $acc_nums =M('acc_nums');
...
function getUserNumsList($uid,$type=1){
    ...
    if($type==1){
        ...
        $where4['aboutid']=$uid;
        $where4['type']= array('lt',20);
        $sale =$acc_num->where($where4)->order('id desc')->select();
        foreach ($sale as $k=>$v){
