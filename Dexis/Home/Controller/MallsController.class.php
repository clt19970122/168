...
//            $price1 = getPrice($post["gnums"]);
        ...
        #
        $post["uid"] = $this->ssid;
        if($post["gsn"] == '815526127943355165'||$post["gsn"] == '615583172840856677'){
    ...
    private function walletPay($data) {
        ...
        //活动的方案------------------------------2019-3-8 20:00:43
        //这个是购买便发货的判断
        $user_sysid =getUserInf($this->ssid,'sysid');
        $status =IsHaveSendParent($user_sysid);
        $pay_type=null;
        if($status){
            $ress =buy_send($this->ssid,$data["sn"]);
            if($ress){
                $pay_type =1;
            }
        }
        //黄金雨
        GoldActivity( $data["sn"],$pay_type);
        //联创的有优惠
