...
class LoginController extends CommController {
    ...
    public function logWechat() {
        $account = M("account");
        $login_information = M("login_information");
        #
        $where["openid"] = $this->wecs["openid"];
        $info = $account->where($where)->find();
        if($info['status']==0){
...
//                session("set_id", $info["id"]);
                ...
                $sysid=$info['sysid'];
                $live=$info['level'];
                //$user_area=$login_information->where(['sysid'=>$sysid])->find();
                // if($user_area==null){
                $information=information($sysid,$live);
                $login_information->add($information);
                // }else{
                //     $information=information($sysid);
                //     $login_information->where(['id'=>$user_area['id']])->save($information);
                // }
...
//            session("ssid_dexis", $info["id"]);
        ...
        // $where["recid"] = session("bang_recid");
