...
class OrdSrsOpera {
    ...
    private function updateOrder() {
        ...
        if ($this->arrs["status"] != 5) {
            $order_sr->commit();
            return get_op_res(1, "更新成功[ST]", null);
        }
        //减少每日抢购库存
                ...
                if ($this->user['level'] < 3) {
                    ...
                    if($upInfo["level"]>2) {
                        ...
                        if($should_pay['level']<=$this->user["level"]){
                            ...
                            //如果大于黄金的
                            if (!cgUserMoney($should_pay["id"], $lev['first']*$this->nums, 1, "REBACK", $this->orde["sn"],true,true,$this->drs_sn)) {
                                $account->rollback();
                                return false;
                            }
...
//            }
        ...
        //提货逻辑$this->user['id']
        $res =setPlanToSend($this->user['id'],$this->orde["id"],$this->drs_sn);
        if(!$res){
            $account->rollback();
            return false;
        }
...
//        $goods = getIDGoodsInfo($this->data["gid"]);
    ...
    private function doShareFinishS($tmp, $money,$num) {
       /* #积分返点
        ...
        $roll = $share * $num;
        if ($roll > 0) {
            if (!cgUserMoney($tmp["id"], $roll, 1, "REBACK", $this->orde["sn"],true,true,$this->drs_sn)) {
                return false;
            }
        }
...
//        $goods = getIDGoodsInfo($this->data["gid"]);
        ...
        if ($roll > 0) {
            if (!cgUserMoney($info["id"], $roll, 1, "REBACK", $this->orde["sn"],true,true,$this->drs_sn)) {
                return false;
            }
        }
        //增加利润记录 2018年12月11日11:31:23 返利
...
//        $save["money"] = $info["money"] + $this->orde['money'];
    ...
    // -----------------2019-3-7 14:48:57 add---------

    /**
     *生成提货订单
     */
    public function ToPickGoods($uid){
        $res_data =setPlanToSend($uid);
        return $res_data;
    }
}
